...
// Fallback downloads generator for robustness
function generateFallbackDownloads(packageName: string): any {
  const history: any[] = [];
  const now = new Date();
  let total = 0;
  for (let i = 365; i >= 0; i--) {
...
// Global fetch helper with retry logic
...
// Curated packages for Rankings Page
...
app.get("/api/search", async (req, res) => {
  ...
  try {
    ...
    cache.set(cacheKey, results, 5 * 60 * 1000); // 5 minutes cache for search
...
app.get("/api/package/*", async (req, res) => {
  // Capture scoped or normal package name
  ...
  try {
    const result = await deduplicator.deduplicate(cacheKey, async () => {
      // Encode package name for registry (scoped packages need to encode the slash)
      const encodedRegistryName = packageName.startsWith("@")
        ? `${packageName.split("/")[0]}%2F${packageName.split("/")[1]}`
        : packageName;
      ...
      // Fetch NPM Download Range
      const downloadsPromise = fetchWithRetry(`https://api.npmjs.org/downloads/range/last-year/${packageName}`)
        .then(async (r) => {
          if (!r.ok) {
            console.warn(`Downloads API failed for ${packageName}. Using fallback.`);
            return generateFallbackDownloads(packageName);
          }
      ...
      // Parse repository info for GitHub
      ...
      // Build Release / Version list and compute time intervals
      const timeData = registry.time || {};
      const versionsList: any[] = [];
      const versionsKeys = Object.keys(registry.versions || {}).reverse(); // newest to oldest
      ...
      const hasSecurityAdvisories = false; // Mocked security advisory status for demo or simple static flags
      ...
      // Calculate health score metrics (out of 100)
      ...
      const responsePayload = {
        name: registry.name || packageName,
        ...
        versionsList: versionsList.slice(0, 50), // Send first 50 versions to keep bundle size lightweight, clients can request more if needed
        downloads,
        github,
        health: {
          score: healthScore,
      ...
      cache.set(cacheKey, responsePayload, 60 * 60 * 1000); // 1 hour cache
  ...
  try {
    const results = await Promise.all(
      packageNames.map(async (name) => {
        try {
          // Internal call to local package API or direct caching lookup
          const encodedName = name.startsWith("@")
            ? `${name.split("/")[0]}%2F${name.split("/")[1]}`
            : name;

          // Call the API endpoint logic or fetch internally
          const host = `http://localhost:${PORT}`;
          const resRaw = await fetchWithRetry(`${host}/api/package/${encodedName}`);
          if (resRaw.ok) {
...
app.get("/api/rankings", async (req, res) => {
  ...
  try {
    // Lazily fetch downloads for POPULAR_PACKAGES and compile them
    const host = `http://localhost:${PORT}`;
    const items = await Promise.all(
      POPULAR_PACKAGES.map(async (pName) => {
        try {
          const resRaw = await fetchWithRetry(`${host}/api/package/${encodeURIComponent(pName)}`);
          if (resRaw.ok) {
    ...
    // Sort valid packages dynamically into different categories
    // Category 1: Most Downloaded This Week
    ...
    // Category 2: Fastest Growing / Trending
    ...
    // Category 3: Community Stars / Popular
    ...
    cache.set(cacheKey, responsePayload, 12 * 60 * 60 * 1000); // 12 hours cache
...
const startServer = async () => {
  if (process.env.NODE_ENV !== "production") {
    ...
    // Serve index.html for React App routing
    app.get("*", (req, res) => {
